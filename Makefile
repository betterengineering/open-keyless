...
build: clean
	CC="arm-linux-gnueabihf-gcc" GOOS=linux GOARCH=arm GOARM=6 CGO_ENABLED=1 go build -o build/out/linux/arm/open-keyless-controller --ldflags '-linkmode external -extldflags "-static"' github.com/lodge93/open-keyless/cmd/open-keyless-controller
