...
build: clean
	CC="tools/arm-bcm2708/arm-linux-gnueabihf/arm-linux-gnueabihf" GOOS=linux GOARCH=arm GOARM=6 CGO_ENABLED=1 CGO_LDFLAGS="-lusb" go build -o build/out/linux/arm/open-keyless-controller --ldflags '-linkmode external -extldflags "-static"' github.com/lodge93/open-keyless/cmd/open-keyless-controller
