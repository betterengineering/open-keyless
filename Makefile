...
build: clean
	CC="tools/arm-bcm2708/arm-rpi-4.9.3-linux-gnueabihf/bin/arm-linux-gnueabihf-gcc" GOOS=linux GOARCH=arm GOARM=6 CGO_ENABLED=1 CGO_LDFLAGS="-lusb" go build -o build/out/linux/arm/open-keyless-controller --ldflags '-linkmode external -extldflags "-static"' github.com/lodge93/open-keyless/cmd/open-keyless-controller
